...
const ProfilePageComponent: React.FC<ProfilePageProps> = ({
  ...
  const renderWordItem = (wordData: ExploredWordEntry) => {
    return (
      <li
        key={wordData.word}
        className="mb-3 p-4 bg-[--hover-bg-color] rounded-lg shadow-sm transition-colors hover:bg-[--border-color]"
      >
        <div className="flex justify-between items-center">
          ...
          <button
            onClick={() => onToggleFavorite(wordData.word, wordData.is_favorite)}
            className={`p-1.5 rounded-full hover:bg-slate-600 transition-colors ${
              wordData.is_favorite
                ? 'text-pink-500'
                : 'text-[--text-tertiary]'
            }`}
            title={wordData.is_favorite ? 'Unfavorite' : 'Favorite'}
          >
            <Heart
              size={18}
              fill={wordData.is_favorite ? 'currentColor' : 'none'}
            />
          </button>
        </div>
        {wordData.last_explored_at && (
          <p className="text-xs text-[--text-tertiary] mt-1">
            Last explored:{' '}
            {new Date(wordData.last_explored_at).toLocaleDateString()}
          </p>
        )}
      </li>
    );
  ...
  const renderStreakItem = (streak: StreakRecord) => (
    <li
      key={streak.id}
      className="mb-3 p-4 bg-[--hover-bg-color] rounded-lg shadow"
    >
    ...
    <div className="w-full">
      ...
      <div className="bg-[--background-secondary] p-6 rounded-lg shadow-xl mb-8">
        ...
        <p className="text-[--text-secondary]">
          Total Words Explored:{' '}
          <span className="font-bold text-[--accent-primary]">
            {isProUser ? userProfileData.totalWordsExplored : 0}
          </span>
        </p>
        <div className="mt-4 pt-4 border-t border-[--border-color]">
          ...
          <p className="text-[--text-secondary]">
            ...
            <span className="font-bold text-green-400">
              {userProfileData.total_quiz_questions_correct || 0}
            </span>
          </p>
          {userProfileData.total_quiz_questions_answered &&
          userProfileData.total_quiz_questions_answered > 0 ? (
            <p className="text-[--text-secondary]">
              Accuracy:{' '}
            ...
            <p className="text-[--text-secondary]">
              Accuracy: <span className="font-bold text-amber-400">N/A</span>
            </p>
          )}
        </div>
      </div>
      ...
      <div className="mb-6">
        <div className="flex border-b border-[--border-color]">
          ...
          >
            All Explored (
            {isProUser ? userProfileData.exploredWords?.length : 0})
          </button>
          <button
          ...
          >
            Streak History (
            {isProUser ? userProfileData.streakHistory?.length : 0})
          </button>
        </div>
      </div>
      ...
      <div className="bg-[--background-secondary] p-6 rounded-lg shadow-xl min-h-[400px]">
        {activeTab === 'explored' && (
          <div>
            ...
            {isProUser ? (
              userProfileData.exploredWords &&
              userProfileData.exploredWords.length > 0 ? (
                <ul className="max-h-[500px] overflow-y-auto pr-2">
                  {userProfileData.exploredWords.map((word) =>
                    renderWordItem(word)
                  )}
                </ul>
              ) : (
                <p className="text-[--text-tertiary]">
                  No words explored yet. Start learning!
                </p>
              )
            ) : (
        ...
        {activeTab === 'favorites' && (
          <div>
            ...
            {isProUser ? (
              userProfileData.favoriteWords &&
              userProfileData.favoriteWords.length > 0 ? (
                <ul className="max-h-[500px] overflow-y-auto pr-2">
                  {userProfileData.favoriteWords.map((word) =>
                    renderWordItem(word)
                  )}
                </ul>
              ) : (
          ...
          <div>
            ...
            {isProUser ? (
              ...
              userProfileData.streakHistory.length > 0 ? (
                ...
                <p className="text-[--text-tertiary]">
                  No completed streaks yet. Keep learning to build them!
                </p>
              )
            ) : (
