...
interface WordContent {
  ...
  modes_generated?: string[];
  word?: string; // Word itself for context
}

// This type represents the structure for a single word's data in the main generatedContent state
// It directly holds the content for each mode.
interface WordDataInState {
  ...
  quiz_progress?: QuizAttempt[];
  modes_generated?: string[];
  // No explicit_connections in your original WordContent, add if needed
}
...
const parseQuizString = (quizStr: string): ParsedQuizQuestion | null => {
  ...
  for (let i = 1; i <= 4; i++) {
    ...
    const match = lines[i].match(optionRegex); // `match` is used here
  ...
  if (correctAnswerLine) {
    const correctMatch = correctAnswerLine.match(/(?:Correct Answer:\s*|^\s*)([A-D])(?:[.)]?\s*.*)?$/i); // `correctMatch` is used here
...
function App() {
  ...
  const [liveStreak, setLiveStreak] = useState<LiveStreak>({ score: 0, words: [] }); // Correct initialization
  ...
  // Adjusted handleAuthSuccess to match backend response (access_token and user object)
  const handleAuthSuccess = (token: string, userDetailsFull?: {username: string, userId: string, email?:string} ) => {
    localStorage.setItem('authToken', token);
    setAuthToken(token);
    if (userDetailsFull) {
      // Construct UserProfile from userDetailsFull
      const profile: UserProfile = { username: userDetailsFull.username, userId: userDetailsFull.userId, email: userDetailsFull.email };
      setCurrentUser(profile);
      // If other profile details are needed immediately, fetchUserProfile can be called,
      // but basic user info is set.
    } else {
      fetchUserProfile(token); 
    }
  ...
  type GenerateExplanationResponse = WordContent & {
    word: string; // The word for which content was generated/fetched
    mode: ContentMode; // The specific mode returned in 'content'
    content: string | string[]; // Content for the specific mode
    generated_content_cache: WordContent; // Full cache for the word from DB
    modes_generated: string[];
    is_favorite: boolean;
    quiz_progress: QuizAttempt[];
  };
  ...
  const handleGenerateExplanation = async (
    ...
    if (!authToken) { setShowAuthModal(true); setAuthMode('login'); setAuthError("Please log in to generate content."); return; }
    ...
    console.log(`Generating content for "${wordToFetch}", mode "${targetMode}"`);
    try {
      ...
      const data: GenerateExplanationResponse = await response.json();
      const wordFromResponse = data.word; // Use word from response as source of truth
      ...
      setGeneratedContent(prev => {
        ...
        // Merge with the full cache from backend, then ensure the specifically fetched mode's content is up-to-date
        const updatedCacheForWord = { ...existingWordData, ...data.generated_content_cache };
        updatedCacheForWord[data.mode] = data.content; // data.mode is the mode for which data.content is relevant

        return {
            ...prev,
            [sanitizedWordId]: {
                ...updatedCacheForWord, // This object has explain, fact, quiz keys with their content
                is_favorite: data.is_favorite, // Use directly from response
                modes_generated: data.modes_generated, // Use directly from response
                quiz_progress: data.quiz_progress // Use directly from response (backend handles reset if needed)
            },
        };
      });

      setActiveContentMode(data.mode); // Set active mode to what was actually fetched/returned
      if (data.mode === 'quiz') { // If quiz was fetched (new or refresh)
      ...
      if (isNewPrimaryWordSearch) {
        ...
        setLiveStreak({ score: 1, words: [wordFromResponse] }); // Option: start streak from profile word
      ...
      console.error("Error generating content:", err); setError((err as Error).message);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleFetchNewQuizSet = () => {
    ...
    if (wordForNewQuiz && authToken) {
        ...
        setShowAuthModal(true); setAuthMode('login'); setAuthError("Please log in to get more questions.");
    }
  };
  ...
  const handleToggleFavorite = async () => { /* ... (Your existing logic) ... */ 
    ...
    try {
      await fetch(`${API_BASE_URL}/toggle_favorite`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${authToken}`}, body: JSON.stringify({ word: currentFocusWord.trim() }), });
  ...
  const handleSubTopicClickInternal = (subTopic: string) => { // Renamed to avoid conflict if any, used by explanationHTML
  ...
  const handleRefreshContent = () => { /* ... (Your existing logic) ... */ 
    if (currentFocusWord) { handleGenerateExplanation(currentFocusWord, false, true, false, activeContentMode); }
  };
  
  const handleWordSelectionFromProfile = (word: string) => { /* ... (Your existing logic) ... */ 
    setShowProfileModal(false); setInputValue(word); 
    handleGenerateExplanation(word, false, false, true, 'explain');
  };

  const handleStreakWordClick = (word: string) => { /* ... (Your existing logic, ensure it doesn't break streak) ... */ 
    ...
    if (generatedContent[sanitizedReviewWord]?.explain) {
      setCurrentFocusWord(word); setCurrentFocusWordSanitized(sanitizedReviewWord); setActiveContentMode('explain'); 
    } else {
  ...
  const handleFetchContentForReview = async (wordToReview: string) => { /* ... (Your existing logic) ... */ 
    ...
    try {
        ...
        const data: GenerateExplanationResponse = await response.json(); // Use the more specific type
        const wordFromResponse = data.word; // Use word from response
        const sanitizedId = sanitizeWordForId(wordFromResponse);
        setGeneratedContent(prev => ({ ...prev, [sanitizedId]: { ...prev[sanitizedId], ...data.generated_content_cache, [data.mode]: data.content, is_favorite: data.is_favorite, modes_generated: data.modes_generated, quiz_progress: data.quiz_progress } }));
  ...
  useEffect(() => { /* ... (Your existing quiz useEffect logic) ... */ 
    ...
    if (activeContentMode === 'quiz' && sanitizedWordInFocus && generatedContent[sanitizedWordInFocus]?.quiz) {
        ...
        const quizSet = wordData.quiz!; 
        const progress = wordData.quiz_progress || [];
        if (quizSet.length > 0) { setCurrentQuizQuestionIndex(progress.length >= quizSet.length ? quizSet.length : progress.length);
  ...
  // FIX for TS6133: Use `_match` for unused parameter
  const explanationHTML = { __html: currentDisplayWordData?.explain?.replace(/<click>(.*?)<\/click>/g, 
    (_match, p1) => `<button class="text-purple-600 hover:text-purple-800 font-semibold underline decoration-dotted hover:decoration-solid" data-subtopic="${p1}">${p1}</button>`) || '' };
  ...
  const renderContent = () => { /* ... (Your existing renderContent logic, ensure subtopic clicks call handleSubTopicClickInternal) ... */ 
    ...
    if (generalErrorToDisplay && !displayData?.[activeContentMode]) return <div className="text-red-500 p-4 bg-red-100 rounded-md">{error}</div>;
    ...
    switch (activeContentMode) {
      case 'explain':
        ...
        return ( <div className="prose max-w-none p-1 text-gray-800" onClick={(e) => {  const target = e.target as HTMLElement; if (target.tagName === 'BUTTON' && target.dataset.subtopic) { handleSubTopicClickInternal(target.dataset.subtopic); } }}>
            ...
            {displayData?.explain && ( <button onClick={handleRefreshContent} className="mt-2 text-xs text-blue-500 hover:text-blue-700 flex items-center" title="Refresh explanation"> <RefreshCw size={12} className="mr-1" /> Regenerate </button> )} </div> );
      case 'fact': return <div className="prose max-w-none p-1 text-gray-800">{displayData?.fact || "No fact available yet."}</div>;
      case 'image': return <div className="prose max-w-none p-1 text-gray-800">{displayData?.image || "Image feature coming soon."}</div>;
      case 'deep_dive': return <div className="prose max-w-none p-1 text-gray-800">{displayData?.deep_dive || "Deep dive feature coming soon."}</div>;
      case 'quiz':
        // ... (Your existing detailed quiz rendering logic from the provided App.tsx)
        // This part is quite long, ensure it's correctly using `wordToDisplay` for titles etc.
        // For brevity, I'm not re-pasting the entire quiz render here but it should be the same as your file.
        // Key is that it uses `displayData` which is for `wordToDisplay`.
        ...
        if (!quizSet || quizSet.length === 0) return <div className="p-4 text-gray-500">No quiz available.</div>;
        if (currentQuizQuestionIndex >= quizSet.length) { /* Summary View */ 
            ...
            return ( <div className="p-4 space-y-4 text-gray-800">  <h3 className="text-xl font-semibold text-gray-700 mb-2">Quiz Summary for "{wordToDisplay}"</h3> <p className="text-lg font-medium mb-3">Your Score: {correctCount} / {quizSet.length}</p> <div className="max-h-[50vh] overflow-y-auto space-y-3 pr-2"> {quizSet.map((quizString, index) => { const parsedQuestion = parseQuizString(quizString); if (!parsedQuestion) return <div key={index} className="text-red-500 text-sm p-2 bg-red-50 rounded">Error Q{index + 1}.</div>; const attempt = quizProgress.find(p => p.question_index === index); return ( <div key={index} className="p-3 border rounded-lg shadow-sm bg-white"> <p className="font-semibold text-gray-700 text-sm mb-1">Q{index + 1}: {parsedQuestion.questionText}</p> <ul className="space-y-1 text-xs"> {parsedQuestion.options.map(opt => ( <li key={opt.key} className={`p-1.5 rounded border ${opt.key === parsedQuestion.correctOptionKey ? 'bg-green-50 border-green-200 font-medium text-green-700' : 'text-gray-600'} ${attempt && opt.key === attempt.selected_option_key && opt.key !== parsedQuestion.correctOptionKey ? 'bg-red-50 border-red-200 text-red-700' : ''} `}> ({opt.key}) {opt.text} {opt.key === parsedQuestion.correctOptionKey && <CheckCircle size={12} className="inline ml-1 text-green-500" />} {attempt && opt.key === attempt.selected_option_key && opt.key !== parsedQuestion.correctOptionKey && <XCircle size={12} className="inline ml-1 text-red-500" />} </li> ))} </ul> {attempt ? (attempt.is_correct ? <p className="mt-1 text-xs text-green-600">Correct.</p> : <p className="mt-1 text-xs text-red-600">Incorrect. Correct: ({parsedQuestion.correctOptionKey})</p>) : <p className="mt-1 text-xs text-orange-400">Not attempted.</p>} </div> ); })} </div> <button onClick={handleFetchNewQuizSet} disabled={isLoading} className="w-full bg-purple-500 hover:bg-purple-600 text-white font-semibold py-2 px-4 rounded-lg flex items-center justify-center disabled:opacity-60"> {isLoading ? <Loader2 className="animate-spin mr-2" size={18}/> : <PlusCircle size={18} className="mr-2" />} More Questions for "{wordToDisplay}" </button> </div> );
        }
        ...
        return ( <div className="p-4 space-y-4 text-gray-800"> <p className="font-semibold text-lg text-gray-700">Q {currentQuizQuestionIndex + 1}/{quizSet.length}:</p> <p className="text-gray-800">{parsedQuestion.questionText}</p> <div className="space-y-2"> {parsedQuestion.options.map(opt => ( <button key={opt.key} onClick={() => !alreadyAnsweredThisQuestion && handleQuizOptionSelect(opt.key, parsedQuestion.correctOptionKey, currentQuizQuestionIndex)} disabled={alreadyAnsweredThisQuestion || isQuizAttempted} className={`w-full text-left p-3 rounded-lg border T ${selectedQuizOption === opt.key ? (quizFeedback?.isCorrect ? 'bg-green-200 BRDRGRN4 RING2 RINGGRN5' : 'bg-red-200 BRDRRD4 RING2 RINGRD5') : 'bg-white hover:bg-gray-100 border-gray-300'} ${alreadyAnsweredThisQuestion && opt.key === attemptForThisQuestion!.selected_option_key ? (attemptForThisQuestion!.is_correct ? 'bg-green-200 BRDRGRN4' : 'bg-red-200 BRDRRD4') : ''} ${alreadyAnsweredThisQuestion && opt.key === parsedQuestion.correctOptionKey && opt.key !== attemptForThisQuestion!.selected_option_key ? 'BRDRGRN5 border-2' : ''} DSBLOP70 DSBLCRSNA`}> ({opt.key}) {opt.text} </button> ))} </div> {(isQuizAttempted || alreadyAnsweredThisQuestion) && quizFeedback && ( <div className={`p-2 RNDMD TXTsm ${quizFeedback.isCorrect ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}> {quizFeedback.message} {!quizFeedback.isCorrect && ` Correct: ${parsedQuestion.correctOptionKey}`} </div> )} {(isQuizAttempted || alreadyAnsweredThisQuestion) && ( <button onClick={handleNextQuestion} className="w-full bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 RNDLG T"> {currentQuizQuestionIndex < quizSet.length - 1 ? 'Next Question' : 'View Summary'} </button> )} <div className="text-xs text-gray-500 mt-2"> Progress: {quizProgress.length}/{quizSet.length}. Score: {quizProgress.filter(p=>p.is_correct).length}. </div> </div> );
      default: return <div className="p-4 text-gray-500">Select a content mode.</div>;
    }
  };
  
  const renderProfileModal = () => { /* ... (Your existing renderProfileModal logic with typed maps) ... */ 
    if (!showProfileModal || !currentUser) return null;
    const renderWordListInternal = (title: string, words: WordHistoryEntry[] | undefined) => ( <div className="mb-4"> <h4 className="font-semibold text-gray-700 mb-1">{title} ({words?.length || 0})</h4> {words && words.length > 0 ? ( <ul className="max-h-40 overflow-y-auto text-sm space-y-1"> {words.map((wh: WordHistoryEntry) => ( <li key={wh.id} onClick={() => handleWordSelectionFromProfile(wh.word)} className="p-1.5 hover:bg-gray-200 rounded cursor-pointer flex justify-between items-center text-gray-800"> <span>{wh.word} <span className="text-xs text-gray-500">({new Date(wh.last_explored_at).toLocaleDateString()})</span></span> {wh.is_favorite && <Heart size={14} className="text-red-500 fill-current" />} </li> ))} </ul> ) : <p className="text-xs text-gray-500">No words here.</p>} </div> );
    const renderStreakListInternal = (streaks: StreakEntry[] | undefined) => ( <div className="mb-4"> <h4 className="font-semibold text-gray-700 mb-1">Streak History ({streaks?.length || 0})</h4> {streaks && streaks.length > 0 ? ( <ul className="max-h-40 overflow-y-auto text-sm space-y-1"> {streaks.map((streak: StreakEntry) => ( <li key={streak.id} className="p-1.5 hover:bg-gray-200 rounded text-gray-800"> <span className="font-medium">Score {streak.score}:</span> {streak.words.map((w: string, i: number) => ( <span key={i} onClick={() => handleWordSelectionFromProfile(w)} className="cursor-pointer hover:underline">{w}</span> )).reduce((prev, curr) => <>{prev} → {curr}</>)} <span className="text-xs text-gray-500 ml-2">({new Date(streak.completed_at).toLocaleDateString()})</span> </li> ))} </ul> ) : <p className="text-xs text-gray-500">No past streaks.</p>} </div> );
  ...
  const renderAuthModal = () => { /* ... (Your existing renderAuthModal logic from App.tsx) ... */ 
    ...
    const handleSubmitAuth = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        // Updated to expect 'access_token' and 'user' object from backend
        if (data.access_token && data.user && data.user.username && data.user.userId) {
            handleAuthSuccess(data.access_token, data.user); 
        } else if (authMode === 'signup' && data.message === 'User created successfully' && data.access_token && data.user) { // Signup might also return user details for immediate login
            handleAuthSuccess(data.access_token, data.user);
        }
         else { throw new Error("Authentication response is missing expected data (access_token or user details)."); }
