...
// --- MainApp contains the existing logic and state ---
function MainApp() {
  ...
  const handleFormSubmit = (e: FormEvent) => {
    ...
    } else if (startMode === 'story_mode') {
      navigate(`/story/${inputValue}`); // Navigate to story mode
    } else if (startMode === 'game_mode') {
      navigate(`/game/${inputValue}`); // Navigate to game mode
    }
    setInputValue('');
  };
  ...
  // --- This is the MAIN VIEW that contains the router ---
  const MainView = () => (
    <div className="flex-grow flex flex-col relative overflow-hidden">
      ...
      {/* Render the input form only on the main routes, not in the game or profile */}
      {(location.pathname === '/') && (
            ...
            <form onSubmit={handleFormSubmit} className="bg-[--background-input] rounded-3xl p-4 flex flex-col shadow-lg border border-transparent focus-within:border-[--accent-primary] transition-colors">
              <div className="flex items-center w-full">
                <input
                  ...
                  placeholder={
                    startMode === 'explore_mode' ? "Enter a word to explore..." : 
                    startMode === 'story_mode' ? "Enter a topic for a story..." : 
              ...
              <div className="flex items-center gap-4 mt-3">
                <button type="button" onClick={() => setStartMode('explore_mode')} className={`flex items-center gap-2 py-1 px-3 rounded-lg text-sm font-medium transition-colors ${startMode === 'explore_mode' ? 'bg-[--accent-primary] text-black' : 'text-[--text-secondary] hover:bg-[--hover-bg-color]'}`}><BookText size={16} /> Explore</button>
                <button type="button" onClick={() => setStartMode('story_mode')} className={`flex items-center gap-2 py-1 px-3 rounded-lg text-sm font-medium transition-colors ${startMode === 'story_mode' ? 'bg-[--accent-primary] text-black' : 'text-[--text-secondary] hover:bg-[--hover-bg-color]'}`}><FileText size={16} /> Story</button>
                <button type="button" onClick={() => setStartMode('game_mode')} className={`flex items-center gap-2 py-1 px-3 rounded-lg text-sm font-medium transition-colors ${startMode === 'game_mode' ? 'bg-[--accent-primary] text-black' : 'text-[--text-secondary] hover:bg-[--hover-bg-color]'}`}><Swords size={16} /> Game</button>
              </div>
            </form>
          </div>
        </div>
  ...
  // A small wrapper component to extract the topic from the URL for StoryMode
