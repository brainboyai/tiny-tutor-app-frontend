...
function App() {
  ...
  const resetChat = useCallback(() => { const action = async () => { if (liveStreak && liveStreak.score > 0 && authToken) { const newHistory = await saveStreakToServer(liveStreak, authToken); if (newHistory && userProfileData) { setUserProfileData(prev => ({...prev!, streakHistory: newHistory})); } } setInputValue(''); setCurrentFocusWord(null); setLiveStreak(null); setLiveStreakQuizQueue([]); setError(null); setIsReviewingStreakWord(false); setIsQuizVisible(false); setIsInitialView(true); }; if (liveStreak && liveStreak.score > 0) { setWarningAction({ action }); setShowWarningModal(true); } else { action(); }}, [liveStreak, authToken, saveStreakToServer, userProfileData]);
      ...
      <div className="flex h-dvh w-full bg-[--background-default] text-[--text-primary] font-sans">
        ...
        <div className="flex flex-col flex-grow h-full max-h-screen">
          ...
          <div className="flex-grow flex flex-col relative overflow-hidden">
            ...
            <div className={`absolute bottom-0 left-0 w-full p-4 bg-gradient-to-t from-[--background-default] to-transparent transition-opacity duration-300 ${isInitialView && activeView === 'main' ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
