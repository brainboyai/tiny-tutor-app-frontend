...
import { User as UserIcon, Mail, BookOpen, Star, ListChecks, ArrowLeft, RefreshCw, Heart } from 'lucide-react';
...
const ProfilePage: React.FC<ProfilePageProps> = ({
    ...
    if (userProfileData.isLoading && !userProfileData.username) { // Show full page loader if initial load
        return (
...
if (userProfileData.error && !userProfileData.username) { // Show full page error if initial load failed
    return (
...
if (!currentUser && !userProfileData.username) { // Should not happen if auth is handled, but as a fallback
    return (
        <div className= "flex-grow container mx-auto px-4 sm:px-6 py-8 flex flex-col items-center justify-center text-center" >
...
const renderWordItem = (item: WordHistoryEntry, isFavoriteList: boolean = false) => (
    <li key= { item.id } className = "p-3 sm:p-4 bg-white dark:bg-slate-700 dark:bg-opacity-50 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-150 border border-slate-200 dark:border-slate-600" >
...

{
    userProfileData.error && (
        <div className="mb-4 p-3 bg-red-100 dark:bg-red-900 dark:bg-opacity-30 border border-red-300 dark:border-red-600 rounded-lg text-red-700 dark:text-red-300 text-sm" >
            Error refreshing profile: { userProfileData.error }
    </div>
      )
}
                    ...
                    <Mail size={ 18 } className = "mr-2.5 text-slate-400 dark:text-slate-500" />
                        ...
                        < div className = "flex items-center text-sm sm:text-base text-slate-600 dark:text-slate-300" >
                            <BookOpen size={ 18 } className = "mr-2.5 text-slate-400 dark:text-slate-500" />
                                <span>Total Words Explored: <span className="font-bold" > { userProfileData.totalWordsExplored } < /span></span >
                                    </div>
                                    </section>
                    ...
                    </div>
{
    userProfileData.exploredWords.length > 0 ? (
        <ul className= "space-y-2.5 sm:space-y-3 overflow-y-auto flex-grow max-h-[50vh] sm:max-h-[60vh] pr-1 nice-scrollbar" >
                        ...
                        </div>
{
    userProfileData.streakHistory.length > 0 ? (
        <ul className= "space-y-3 sm:space-y-4 overflow-y-auto flex-grow max-h-[50vh] sm:max-h-[60vh] pr-1 nice-scrollbar" >
