...
// Removed unused imports: User, Star, List, BookOpen, ChevronLeft, Menu, Settings
import { LogIn, LogOut, Search, Send, RefreshCw, Heart, CheckCircle, XCircle, ChevronRight, Image as ImageIcon, Brain, X, User as UserIcon, Info, Moon, Sun, Sparkles, FileText, Edit2 } from 'lucide-react';
...
function App() {
  ...
  const [isLoading, setIsLoading] = useState(false); // Global loading for content generation
  ...
  const [isAuthProcessing, setIsAuthProcessing] = useState(false); // Specific loading state for auth form
        ...
        <div className="bg-white dark:bg-slate-800 p-6 sm:p-8 rounded-xl shadow-2xl w-full max-w-md relative">
          ...
          <form onSubmit={async (e: React.FormEvent<HTMLFormElement>) => { // Typed e
            ...
            <button type="submit" disabled={isAuthProcessing} className="w-full bg-sky-600 hover:bg-sky-700 text-white font-semibold py-3 px-4 rounded-lg transition duration-150 ease-in-out focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 dark:focus:ring-offset-slate-800 disabled:opacity-70 flex items-center justify-center">
              {isAuthProcessing ? <RefreshCw size={20} className="animate-spin mr-2" /> : (authMode === 'login' ? <LogIn size={20} className="mr-2" /> : <UserIcon size={20} className="mr-2" />)}
              {isAuthProcessing ? 'Processing...' : (authMode === 'login' ? 'Login' : 'Sign Up')}
            </button>
          </form>
