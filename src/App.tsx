...
function App() {
  ...
  // --- Core Logic (Unchanged functions) ---
  ...
  // --- MODIFIED: `handleGenerateExplanation` to preserve state on rate limit ---
  const handleGenerateExplanation = useCallback(async (wordToFetch: string, isNewPrimaryWordSearch: boolean = false, isUserRefreshClick: boolean = false, isSubTopicClick: boolean = false) => {
    ...
    try {
        ...
        if (authToken && isNewPrimaryWordSearch && userProfileData) {
            ...
            if (!wordExists) {
                setUserProfileData(prev => {
                    if (!prev) return null;
                    return { ...prev, exploredWords: [newWordEntry, ...prev.exploredWords], totalWordsExplored: prev.totalWordsExplored + 1, };
                });
            }
        }
        ...
        // MODIFICATION: Only set the main error state if it's NOT our specific rate limit error.
        if (err instanceof Error && err.message !== RATE_LIMIT_ERROR_MESSAGE) {
            setError(err.message);
        } else {
            console.error(err); // Log the rate limit error but don't wipe the UI
        }
    } finally {
        setIsLoading(false);
  ...
  // --- RENDER FUNCTIONS (Unchanged unless specified) ---
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
            <div className="bg-[--background-secondary] p-8 rounded-xl shadow-2xl w-full max-w-md relative">
                ...
                <div className="flex flex-col space-y-3">
                    <button 
                      onClick={() => { setShowRateLimitModal(false); setShowSettingsModal(true); }}
    ...
    const handleValidateAndSave = async () => {
        ...
        try {
            ...
            if (err instanceof Error) {
                ...
                setApiKeyValidationStatus({ message: 'An unknown error occurred.', type: 'error' });
            }
        } finally {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
            <div className="bg-[--background-secondary] p-8 rounded-xl shadow-2xl w-full max-w-lg relative">
                <button onClick={closeModalAndReset} className="absolute top-4 right-4 text-[--text-tertiary] hover:text-[--text-primary]"><X size={24} /></button>
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <input
                            ...
                            placeholder="Enter your key to bypass daily limits"
                        />
                    ...
                    <div className="flex justify-end gap-4 pt-4">
                        <button onClick={handleValidateAndSave} className="py-2 px-6 rounded-md text-sm bg-[--accent-primary] text-black font-semibold disabled:opacity-50" disabled={isApiKeyValidating}>
  ...
  // --- MODIFIED: `renderMainContent` to pass rate limit handler to child components ---
  const renderMainContent = () => {
    ...
    if (activeGameMode === 'story_mode' && activeTopic) {
      return <StoryModeComponent topic={activeTopic} authToken={authToken} onStoryEnd={resetChat} language={language} onRateLimitExceeded={() => setShowRateLimitModal(true)} />;
    }
