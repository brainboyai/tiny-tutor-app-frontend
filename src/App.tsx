...
function App() {
  ...
  const resetChat = () => { const action = async () => { if (liveStreak && liveStreak.score > 0 && authToken) { const newHistory = await saveStreakToServer(liveStreak, authToken); if (newHistory && userProfileData) { setUserProfileData(prev => ({...prev!, streakHistory: newHistory})); } } setInputValue(''); setCurrentFocusWord(null); setLiveStreak(null); setLiveStreakQuizQueue([]); setError(null); setIsReviewingStreakWord(false); setIsQuizVisible(false); setIsInitialView(true); }; if (liveStreak && liveStreak.score > 0) { setWarningAction({ action }); setShowWarningModal(true); } else { action(); }};
