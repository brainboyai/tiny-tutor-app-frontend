...
function DraggableQuizButton({ unattemptedCount, onClick }) {
...
function App() {
 ...
 <React.Fragment key={word + index}>
                          <button 
                            className={`py-1 px-3 rounded-full text-sm font-medium transition-all duration-300 ${getDisplayWord() === word ? 'bg-sky-600 text-white shadow-md' : 'bg-[--background-secondary] hover:bg-[--hover-bg-color]'}`} 
 ...
 {word}
              ...
              {activeView === 'profile' && <ProfilePageComponent currentUser={currentUser!} userProfileData={userProfileData} onWordSelect={handleWordSelectionFromProfile} onToggleFavorite={handleToggleFavorite} onNavigateBack={() => setActiveView('main')} generatedContent={generatedContent} />}
 <div ref={chatEndRef} />
 </div>
 {renderQuizPopup()}
 </main>
