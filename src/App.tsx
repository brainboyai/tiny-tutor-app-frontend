...
function App() {
  ...
  const handleToggleFavorite = useCallback(async (word: string, currentStatus: boolean) => { if (!authToken || !word) return; const wordId = sanitizeWordForId(word); setGeneratedContent(prev => ({...prev, [wordId]: { ...(prev[wordId] || {}), is_favorite: !currentStatus }})); setUserProfileData(prev => { if (!prev) return null; return { ...prev, exploredWords: prev.exploredWords.map(w => w.word === word ? { ...w, is_favorite: !currentStatus } : w), favoriteWords: !currentStatus ? [...prev.favoriteWords, prev.exploredWords.find(w => w.word === word)!] : prev.favoriteWords.filter(w => w.word !== word) }; }); try { await fetch(`${API_BASE_URL}/toggle_favorite`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${authToken}` }, body: JSON.stringify({ word }) }); } catch (err: any) { console.error("Error toggling favorite:", err); if (authToken) await fetchUserProfile(authToken); } }, [authToken, fetchUserProfile]);
  const handleWordSelectionFromProfile = useCallback((word: string) => { const action = async () => { if (liveStreak && authToken) { await saveStreakToServer(liveStreak, authToken); await fetchUserProfile(authToken); } setActiveView('main'); setInputValue(word); handleGenerateExplanation(word, true); }; if (liveStreak && liveStreak.score > 0) { setWarningAction({ action }); setShowWarningModal(true); } else { action(); } }, [liveStreak, handleGenerateExplanation, saveStreakToServer, authToken, fetchUserProfile]);
  ...
  const resetChat = () => { const action = async () => { if (liveStreak && authToken) { await saveStreakToServer(liveStreak, authToken); await fetchUserProfile(authToken); } setInputValue(''); setCurrentFocusWord(null); setLiveStreak(null); setLiveStreakQuizQueue([]); setError(null); setIsReviewingStreakWord(false); setIsQuizVisible(false); setIsInitialView(true); }; if (liveStreak && liveStreak.score > 0) { setWarningAction({ action }); setShowWarningModal(true); } else { action(); }};
      ...
      <div className="flex h-dvh w-full bg-[--background-default] text-[--text-primary] font-sans">
        ...
        <div className="flex flex-col flex-grow h-full max-h-screen">
          ...
          <div className="flex-grow flex flex-col relative overflow-hidden">
            <main className={`flex-grow overflow-y-auto transition-all duration-300 ${isInitialView ? 'pb-8' : 'pb-48'}`}>
