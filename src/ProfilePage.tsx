...
import { User, Mail, ShieldCheck, TrendingUp, List, Star, CalendarDays, LogOut, ArrowLeft, Edit3, Save, XCircle } from 'lucide-react';
...
interface ProfilePageProps {
  ...
  onNavigateBack: () => void;
  onLogout: () => void;
  fetchUserProfile: () => void; // To refresh profile data after updates
}
...
const ProfilePage: React.FC<ProfilePageProps> = ({ userProfile, onNavigateBack, onLogout, fetchUserProfile }) => {
  ...
  const handleUsernameUpdate = async () => {
    ...
    setIsLoading(true);
    setEditError(null);
    // This is a placeholder for API call. You'll need to implement this.
    ...
    setTimeout(() => { // Simulate API call
        // setUserProfile(prev => prev ? {...prev, username: newUsername.trim()} : null); // This should be handled by fetchUserProfile
        fetchUserProfile();
        setIsEditingUsername(false);
        setIsLoading(false);
    }, 1000);
  ...
  const favoriteWords = userProfile.favorite_words || exploredWords.filter(w => w.is_favorite); // Fallback if favorite_words isn't a separate list
    ...
    <div className="min-h-screen bg-slate-100 text-slate-800 p-4 sm:p-6 md:p-8">
      ...
      <div className="max-w-4xl mx-auto space-y-6 sm:space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            ...
            {/* Placeholder for current streak - to be developed with gamification */}
            <div className="bg-white p-5 rounded-xl shadow-lg flex items-center">
                <CalendarDays size={28} className="mr-4 text-orange-500" />
                <div>
                    <p className="text-3xl font-bold text-slate-700">{streakHistory.length > 0 ? `${streakHistory[streakHistory.length -1].words_explored_count} words` : 'N/A'}</p> {/* Example: Show last streak day's count */}
          ...
          {favoriteWords.length > 0 ? (
            <ul className="space-y-2 max-h-80 overflow-y-auto custom-scrollbar pr-2">
              {favoriteWords.map(word => (
                <li key={word.id} className="p-3 bg-slate-50 rounded-md hover:bg-slate-100 transition-colors flex justify-between items-center">
                  <span className="font-medium text-slate-700">{word.word}</span>
                  <span className="text-xs text-slate-500">Favorited: {formatDate(word.last_explored_at)}</span> {/* Assuming favorited at last explored */}
                </li>
              ))}
            </ul>
          ) : (
        ...
        <div className="bg-white p-5 sm:p-6 rounded-xl shadow-lg">
          ...
          {streakHistory.length > 0 ? (
             ...
             {streakHistory.slice(-10).reverse().map(streak => ( // Show last 10, newest first
        ...
        <div className="bg-white p-5 sm:p-6 rounded-xl shadow-lg">
            ...
            {/* Add links for "Change Password", "Manage Subscription (Tier)" etc. later */}
            <button 
                onClick={onLogout}
