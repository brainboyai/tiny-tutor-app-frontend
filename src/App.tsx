...
const renderQuizPopup = () => {
    // The entry condition is now simpler: only check if the user has opened the quiz.
    if (!isQuizVisible) return null;
    
    const currentItem = liveStreakQuizQueue[currentStreakQuizItemIndex];
    
    // If there is no item to display (e.g., quiz closed as queue was cleared), exit.
    ...
    return (
      <div className="absolute top-4 right-4 w-full max-w-md bg-[--background-secondary] rounded-lg shadow-2xl p-6 z-20 border border-[--border-color] animate-fadeIn">
        <div className="flex justify-between items-center mb-4">
          <h3 className="font-semibold text-lg text-[--text-primary]">Quiz Time!</h3>
          ...
          {Object.entries(quizQuestion.options).map(([key, text]) => {
            ...
            if (attempted) {
              if (key === quizQuestion.correctOptionKey) {
                buttonColor = "bg-green-800/80"; // Correct answer
              } else if (key === selectedOptionKey) {
                buttonColor = "bg-red-800/80"; // Incorrect answer selected by user
              } else {
                buttonColor = "bg-[--hover-bg-color] opacity-60"; // Other incorrect options
              }
            }
            return (<button key={key} onClick={() => handlePopupQuizAnswer(key)} disabled={attempted} className={`w-full text-left p-3 rounded-md transition-colors ${buttonColor}`}> {text} </button>);
          })}
        </div>
        
        {/* This logic for the Next/Finish button is now reachable */}
        {attempted && (
...
>
  {/* Top row: Input and Submit/Mic button (No changes here) */}
  ...
  {/* --- MODIFIED: Bottom row with updated button styling --- */}
  <div className="flex items-center gap-4 mt-3">
     <button
       ...
       className={`flex items-center gap-2 py-1 px-3 rounded-lg text-sm font-medium transition-colors ${
         startMode === 'word_game'
           ? 'bg-[--accent-primary] text-black' // Prominent style for selected
           : 'text-[--text-secondary] hover:bg-[--hover-bg-color] hover:text-[--text-primary]' // Subtle style for unselected
       }`}
     >
     ...
     >
       <FileText size={16} /> Interactive Mode
     </button>
  </div>
</form>
    </div>
