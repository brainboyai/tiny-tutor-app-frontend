...
import WebContextDisplay from './WebContextDisplay.tsx';
...
// UPDATED: 'suggestions' is present, matching the new requirement.
...
const API_BASE_URL = '/api'; // Using the Render rewrite rule
...
function App() {
  ...
  // UPDATED: Add 'web_agent' to the possible modes
  const [activeGameMode, setActiveGameMode] = useState<'explore_mode' | 'story_mode' | 'game_mode' | 'web_agent' | null>(null);
  ...
  // (The rest of the state definitions are unchanged)
  ...
  const [guestGenerations, setGuestGenerations] = useState<number>(() => { return parseInt(sessionStorage.getItem('guestGenerationCount') || '0', 10); });
  const [storyGuestGenerations, setStoryGuestGenerations] = useState<number>(() => { return parseInt(sessionStorage.getItem('storyGuestGenerationCount') || '0', 10); });
  const GUEST_GENERATION_LIMIT = 3;

  // --- Core Logic Functions (Existing logic preserved with updates) ---
  const handleRateLimit = () => { setShowRateLimitModal(true); };
  const handleGuestLimitExceeded = (message: string) => { setAuthError(message); setShowAuthModal(true); };
  ...
  const handleLogout = useCallback(async () => { if (liveStreak && liveStreak.score >= 2 && authToken) { await saveStreakToServer(liveStreak, authToken); } localStorage.removeItem('authToken'); localStorage.removeItem('customApiKey'); localStorage.removeItem('tiny-tutor-language'); sessionStorage.removeItem('storyModeState'); sessionStorage.removeItem('storyPendingAction'); sessionStorage.removeItem('explorePendingAction'); sessionStorage.removeItem('tiny-tutor-guest-state'); sessionStorage.removeItem('guestGenerationCount'); sessionStorage.removeItem('storyGuestGenerationCount'); setAuthToken(null); setCustomApiKey(''); setCurrentUser(null); setUserProfileData(null); setLiveStreak(null); setCurrentFocusWord(null); setGeneratedContent({}); setError(null); setAuthError(null); setAuthSuccessMessage(null); setShowAuthModal(false); setActiveView('main'); setLiveStreakQuizQueue([]); setActiveGameMode(null); setActiveTopic(null); setIsInitialView(true); setGuestGenerations(0); setStoryGuestGenerations(0); setLanguage('en'); }, [liveStreak, authToken, saveStreakToServer]);
  ...
  // UPDATED: handleGenerateExplanation now receives and stores the new 'suggestions' array
  const handleGenerateExplanation = useCallback(async (wordToFetch: string, isNewPrimaryWordSearch: boolean = false, isUserRefreshClick: boolean = false, isSubTopicClick: boolean = false) => {
    ...
    try {
        ...
        // NOTE: The backend endpoint is now just '/generate_explanation' for simplicity,
        // but the request body might determine what gets returned.
        const requestBody = { 
          word: wordToFetch, 
          streakContext: streakContextForAPI, 
          language: language,
          // Retaining logic from old file for refresh/contextual explanations
  ...
  // (All other logic functions are preserved from the old file)
  useEffect(() => { if (isResumingFromRateLimit) { if (activeGameMode === 'explore_mode' || (isInitialView && startMode === 'explore_mode')) { const pendingActionJSON = sessionStorage.getItem('explorePendingAction'); if (pendingActionJSON) { const pendingAction = JSON.parse(pendingActionJSON); handleGenerateExplanation( pendingAction.wordToFetch, pendingAction.isNewPrimaryWordSearch, pendingAction.isUserRefreshClick, pendingAction.isSubTopicClick ); sessionStorage.removeItem('explorePendingAction'); } } setIsResumingFromRateLimit(false); } }, [isResumingFromRateLimit, activeGameMode, isInitialView, startMode, handleGenerateExplanation]);
  ...
  const handleValidateAndSaveApiKey = async () => { if (!customApiKey) { localStorage.removeItem('customApiKey'); setApiKeyValidationStatus({ message: 'Key removed from local storage.', type: 'success' }); return; } setIsApiKeyValidating(true); setApiKeyValidationStatus({ message: '', type: 'idle' }); try { const response = await fetch(`${API_BASE_URL}/validate_api_key`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ api_key: customApiKey }) }); const data = await response.json(); if (!response.ok) { throw new Error(data.message || 'Validation failed'); } localStorage.setItem('customApiKey', customApiKey); setApiKeyValidationStatus({ message: data.message, type: 'success' }); setIsResumingFromRateLimit(true); setTimeout(() => { setApiKeyValidationStatus({ message: '', type: 'idle' }); setShowRateLimitModal(false); setActiveView('main'); }, 1500); } catch (err) { if (err instanceof Error) { setApiKeyValidationStatus({ message: err.message, type: 'error' }); } else { setApiKeyValidationStatus({ message: 'An unknown error occurred.', type: 'error' }); } } finally { setIsApiKeyValidating(false); } };
  const handleDeleteAccount = async () => { if (!authToken) { alert("You must be logged in to delete your account."); return; } setIsLoading(true); try { const response = await fetch(`${API_BASE_URL}/delete_account`, { method: 'POST', headers: { 'Authorization': `Bearer ${authToken}` }, }); const data = await response.json(); if (!response.ok) throw new Error(data.error || "Failed to delete account."); alert("Account deleted successfully."); handleLogout(); } catch (err) { if (err instanceof Error) alert(`Error: ${err.message}`); else alert("An unknown error occurred during account deletion."); } finally { setIsLoading(false); } };
  
  // --- RENDER FUNCTIONS ---
  
  // UPDATED: renderExploreModeContent now renders the agentic suggestion buttons
  const renderExploreModeContent = () => {
    ...
    if (error && activeGameMode !== 'web_agent') return <div className="text-center p-10 text-red-400">Error: {error}</div>;
    ...
    return (
      <div className="bg-transparent p-1 animate-fadeIn">
        {/* Explanation and Image rendering is unchanged */}
  ...
  const renderWebAgentView = () => {
    return (
      <div className="animate-fadeIn">
        <div className="mb-6">
          <button 
            onClick={() => { setActiveGameMode('explore_mode'); setError(null); }} 
  ...
  // The other render functions (modals, quiz) are preserved as is
  ...
  const renderRateLimitModal = () => { if (!showRateLimitModal) return null; return ( <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4"> <div className="bg-[--background-secondary] p-8 rounded-xl shadow-2xl w-full max-w-md relative"> <button onClick={() => setShowRateLimitModal(false)} className="absolute top-4 right-4 text-[--text-tertiary] hover:text-[--text-primary]"><X size={24} /></button> <div className="text-center"> <AlertTriangle className="mx-auto h-12 w-12 text-amber-400 mb-4" /> <h2 className="text-2xl font-bold text-center text-[--text-primary] mb-4">Daily Limit Reached</h2> <p className="text-[--text-secondary] mb-6">You've used all your free generations for today. Provide your own key for unlimited use or upgrade to Pro.</p> </div> <div className="flex flex-col space-y-3"> <button onClick={() => { setShowRateLimitModal(false); setActiveView('settings'); }} className="w-full bg-[--accent-primary] hover:bg-[--accent-secondary] text-black font-semibold p-3 rounded-lg transition-colors" > Use Your Own API Key </button> <button onClick={() => { alert("Upgrade to Pro coming soon!"); setShowRateLimitModal(false); }} className="w-full bg-amber-600 hover:bg-amber-500 text-white font-bold p-3 rounded-lg transition-colors" > Upgrade to Pro </button> </div> </div> </div> ); };

  // UPDATED: renderMainContent now decides which mode to show
  const renderMainContent = () => {
    ...
    switch (activeGameMode) {
      ...
      case 'explore_mode':
      default:
        // This includes the initial view (isInitialView) and regular explore mode.
        // We preserve the streak bar display here as it belongs to the explore experience.
        return (
  ...
  // --- Main Return Block ---
  // (The main return block JSX structure is unchanged)
  return (
