...
// --- SUB-COMPONENTS (Consolidated for clarity) ---
...
function App() {
    ...
    try {
        ...
        if (!response.ok) {
            if (response.status === 429) {
                if (authToken) { throw new Error("429: Free daily limit reached. Please upgrade to Pro or provide your own API key in Settings for unlimited generations."); } 
                else { throw new Error("429: Free daily limit reached. Please sign in to upgrade to Pro or use your own API key for unlimited generations."); }
            }
        ...
        if(err instanceof Error) {
            if (err.message.startsWith('429:')) { setShowRateLimitModal(true); } else { setError(err.message); }
  ...
  const handleFormSubmit = (e: FormEvent) => {
    ...
    if (startMode === 'explore_mode') { handleGenerateExplanation(inputValue, true); } 
    else if (startMode === 'story_mode') { setIsInitialView(false); setActiveGameMode('story_mode'); setActiveTopic(inputValue); setInputValue(''); } 
    else if (startMode === 'game_mode') { setIsInitialView(false); setActiveGameMode('game_mode'); setActiveTopic(inputValue); setInputValue(''); }
  };
  ...
  // NEW: Function to render the Settings Modal
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <input
                            ...
                            placeholder="Enter your own API key for unlimited generations"
                        />
                        <p className="text-xs text-[--text-tertiary] mt-2">
                            Providing your own key bypasses the free daily limit. Your key is stored securely in your browser and never sent to our servers.
                        </p>
                    </div>
