...
function App() {
  ...
  const handleGenerateExplanation = useCallback(async (
    ...
    if (targetModeForThisCall !== 'quiz' || (targetModeForThisCall === 'quiz' && liveStreakQuizQueue.length > 0 && !isNewPrimaryWordSearch && !isProfileWordClick) ) {
    ...
    const isContextualExplainCall = targetModeForThisCall === 'explain' && streakContextForAPI.length > 0; // Define earlier
    ...
    if (isNewPrimaryWordSearch || isProfileWordClick) {
        ...
        // ... (streak context population logic - same)
        if (liveStreak && liveStreak.words.length > 0) { const targetWordForContext = isReviewingStreakWord && wordForReview ? wordForReview : wordToFetch; const wordIndexInStreak = liveStreak.words.indexOf(targetWordForContext); if (isSubTopicClick && (!liveStreak.words.includes(wordToFetch) || wordToFetch === liveStreak.words[liveStreak.words.length - 1])) { streakContextForAPI = [...liveStreak.words]; } else if (wordIndexInStreak !== -1) { streakContextForAPI = liveStreak.words.slice(0, wordIndexInStreak); } else if (isSubTopicClick) { streakContextForAPI = [...liveStreak.words]; }}
    } else if (currentFocusWord && !isNewPrimaryWordSearch && !isProfileWordClick) {
        if (liveStreak && liveStreak.words.includes(currentFocusWord)) { const currentFocusWordIndex = liveStreak.words.indexOf(currentFocusWord); if (currentFocusWordIndex > 0) streakContextForAPI = liveStreak.words.slice(0, currentFocusWordIndex); }
    }
    
    // Re-calculate isContextualExplainCall after streakContextForAPI is set
    const isActuallyContextual = targetModeForThisCall === 'explain' && streakContextForAPI.length > 0;
    ...
    if (targetModeForThisCall === 'quiz' && liveStreakQuizQueue.length === 0) {
        // ... (single word quiz fetch logic - largely same, ensure explanationForQuiz is robustly obtained)
        let explanationForQuiz = generatedContent[wordId]?.explanation;
        if (!explanationForQuiz && wordToFetch) {
            console.log(`Quiz for ${wordToFetch} needs explanation. Fetching explanation first.`);
            try {
                ...
                explanationForQuiz = explData.explain; // Use the newly fetched explanation
                if (explanationForQuiz) {
                    setGeneratedContent(prev => ({...prev, [wordId]: {...(prev[wordId] || {}), explanation: explanationForQuiz, last_explored_at: new Date().toISOString(), modes_generated: Array.from(new Set([...(prev[wordId]?.modes_generated || []), 'explain'])) }}));
                } else {
                     setError(`Failed to get explanation for "${wordToFetch}" to generate quiz.`); setIsLoading(false); return;
                }
            } catch (err: any) { setError((err as Error).message); console.error("Error pre-fetching explanation for quiz:", err); setIsLoading(false); return; }
        }
        if (!explanationForQuiz) { setError(`Explanation for "${wordToFetch}" is needed. Please view it first.`); setIsLoading(false); return; }
        try {
            ...
            const data = await response.json(); const parsedQuiz = parseQuizStringToArray(data.quiz);
        ...
        } catch (err: any) { setError((err as Error).message); } finally { setIsLoading(false); }
        return; 
    }
    
    // --- Frontend Cache Check for non-quiz, non-forced-refresh requests ---
    let serveFromCache = false;
    if (!isRefreshClick && targetModeForThisCall !== 'quiz') {
        const currentWordItem = generatedContent[wordId];
        if (currentWordItem) {
            if (targetModeForThisCall === 'explain' && typeof currentWordItem.explanation === 'string') {
                serveFromCache = true;
    ...
    if (serveFromCache) {
        ...
        if (isNewPrimaryWordSearch || isProfileWordClick) {
             if (!liveStreak || !liveStreak.words.includes(wordToFetch)) { 
    ...
    try {
        // For API call, refresh_cache true if user explicitly refreshed OR it's a contextual call for 'explain'
        const finalRefreshPolicyForAPI = isRefreshClick || isActuallyContextual;
        ...
        if (targetModeForThisCall === 'explain' && data.explain !== undefined) {
            explanationJustFetched = data.explain;
        }
        ...
        setGeneratedContent(prev => { /* ... (Update generatedContent for non-quiz items) ... */ 
            ...
            if (targetModeForThisCall === 'explain' && data.explain !== undefined) { newWordData.explanation = data.explain; }
            ...
            else if (targetModeForThisCall === 'image') { if (data.image_url !== undefined) { newWordData.image_url = data.image_url; if (data.image_prompt !== undefined) newWordData.image_prompt = data.image_prompt; } else if (data.image !== undefined) { newWordData.image_prompt = data.image; newWordData.image_url = undefined; } }
            ...
            const currentModesGenerated = new Set(existingWordData.modes_generated || []); currentModesGenerated.add(targetModeForThisCall);
        ...
        const isNewWordAddedToStreak = isNewPrimaryWordSearch || isProfileWordClick || 
                                   (isSubTopicClick && (!liveStreak || !liveStreak.words.includes(wordToFetch) || liveStreak.words[liveStreak.words.length-1] !== wordToFetch ));

        if (explanationJustFetched && authToken && isNewWordAddedToStreak) {
            try {
                const quizResponse = await fetch(`${API_BASE_URL}/generate_explanation`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${authToken}` }, body: JSON.stringify({ word: wordToFetch, mode: 'quiz', explanation_text: explanationJustFetched, refresh_cache: true }), });
                if (!quizResponse.ok) { console.error("Failed to fetch quiz for streak item:", (await quizResponse.json()).error); }
                else {
                    const quizData = await quizResponse.json(); const parsedQuiz = parseQuizStringToArray(quizData.quiz);
                    if (parsedQuiz.length > 0) {
                        const newStreakQuizItem: StreakQuizItem = { word: wordToFetch, originalExplanation: explanationJustFetched, quizQuestion: parsedQuiz[0], attempted: false,};
        ...
        if (isNewPrimaryWordSearch || isProfileWordClick) {
            // setCurrentFocusWord(wordToFetch); // Already set
            setLiveStreak({ score: 1, words: [wordToFetch] });
            // setIsReviewingStreakWord(false); setWordForReview(null); // Already set
        } else if (isSubTopicClick) {
             if (isNewWordAddedToStreak) { // only update streak if it's genuinely new
  ...
  }, [authToken, activeContentMode, generatedContent, liveStreak, saveStreakToServer, handleLogout, currentFocusWord, isReviewingStreakWord, wordForReview, error, liveStreakQuizQueue.length]); // Ensure all dependencies are listed
  ...
  const handleStreakWordClick = useCallback((clickedWord: string) => {
    ...
    if (activeContentMode === 'quiz') {
        ...
        else { handleGenerateExplanation(clickedWord, false, false, false, 'explain');  } // isRefreshClick false
    } else { handleGenerateExplanation(clickedWord, false, false, false, 'explain'); } // isRefreshClick false
  ...
  const handleModeChange = useCallback((newMode: ContentMode) => {
    // ... (same, calls HGE with isRefreshClick=false for non-quiz, or true for single-word quiz)
    ...
    if (newMode === 'quiz') {
        if (liveStreakQuizQueue.length > 0) {
            ...
            handleGenerateExplanation(wordToUse, false, true, false, 'quiz'); // isRefreshClick=true for fresh single quiz
        }
    } else if (wordToUse) { 
        handleGenerateExplanation(wordToUse, false, false, false, newMode); // isRefreshClick=false for cache attempt
    } else if (newMode === 'explain') { setActiveContentMode('explain');}
  }, [getDisplayWord, inputValue, handleGenerateExplanation, liveStreakQuizQueue, activeContentMode]); // Added activeContentMode
  
  const handleRefreshContent = useCallback(() => { 
    // ... (same, only for explain)
