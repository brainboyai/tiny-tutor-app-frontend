...
const TinyTutorAppContent: React.FC = () => {
  ...
  interface QuizAttempt {
    question_index: number; // 0-indexed
  ...
  interface GeneratedContent {
    ...
    image?: string;
    fact?: string;
    quiz?: string[]; // Array of quiz strings
    quiz_progress?: QuizAttempt[]; // Array of saved quiz attempts
    deep?: string;
  }
  ...
  interface ExploredWord {
    ...
    explicit_connections?: string[];
    modes_generated?: string[];
    generated_content_cache?: Partial<GeneratedContent>; // Includes quiz and quiz_progress
  }
  ...
  interface InteractiveQuizProps {
    quizText: string;
    onLoadNextQuestion: () => void;
    onQuestionAnswered: (questionIndex: number, selectedOptionKey: string, isCorrect: boolean) => void; // Updated
    canLoadNext: boolean;
    questionNumber: number; // 1-indexed for display
    questionIndex: number; // 0-indexed for logic
    totalQuestionsInSet: number;
    previousAttempt: QuizAttempt | null; // New prop
  }
  ...
  const InteractiveQuiz: React.FC<InteractiveQuizProps> = ({
    ...
    useEffect(() => {
      // Reset and parse when quizText or previousAttempt changes
      const parseAndSetQuiz = (text: string, attempt: QuizAttempt | null) => {
        ...
        if (question && options.length >= 2 && correctAnswerKey && options.some(opt => opt.key === correctAnswerKey)) {
          ...
          // If there's a previous attempt for this specific question index, apply it
    ...
    }, [quizText, previousAttempt, questionIndex]); // questionIndex ensures re-eval if it changes for same text (unlikely but good practice)

    const handleOptionSelect = (optionKey: string) => {
      if (isAnswered || !parsedQuiz) return; // Prevent re-answering if already answered (from current session or previousAttempt)

      setSelectedOptionKey(optionKey);
      setIsAnswered(true);
      const isCorrect = optionKey === parsedQuiz.correctAnswerKey;

      // Notify parent, passing the 0-indexed questionIndex
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          {parsedQuiz.options.map((option) => {
            ...
            const isCorrect = option.key === parsedQuiz.correctAnswerKey;
            let buttonClass = "w-full text-left p-3 border rounded-md transition-all duration-150 flex justify-between items-center ";
            if (isAnswered) { // This 'isAnswered' is the component's local state, reflecting current or past answer
              if (isCorrect) buttonClass += "bg-green-100 dark:bg-green-700 border-green-500 dark:border-green-400 text-green-700 dark:text-green-200 font-semibold";
              else if (isSelected && !isCorrect) buttonClass += "bg-red-100 dark:bg-red-700 border-red-500 dark:border-red-400 text-red-700 dark:text-red-200";
              else buttonClass += "bg-gray-100 dark:bg-gray-600 border-gray-300 dark:border-gray-500 text-gray-700 dark:text-gray-300 cursor-not-allowed opacity-70";
            } else {
  ...
  const TinyTutorAppContent: React.FC = () => {
    ...
    // quizSessionScores is removed, we'll derive score from generatedContents.quiz_progress

    const resetQuizStateForNewWord = () => {
      setCurrentQuizDisplayIndex(0);
      // quiz_progress will be part of generatedContents, so it's reset when generatedContents is cleared
    };
    ...
    const generateContent = async (question: string, mode: ContentMode, options: { isNewPrimaryFocus?: boolean; triggeredBy?: 'generate_btn' | 'refresh_btn' | 'sub_topic_click' | 'profile_click' | 'mode_toggle' | 'review_click' | 'past_streak_click'; isReview?: boolean; preserveCurrentStreak?: boolean; } = {}) => {
      ...
      try {
        ...
        const data = await response.json();
        if (!response.ok) {
          // Even on error, backend might send full_cache with potentially useful old data
          if (data.full_cache) setGeneratedContents(data.full_cache);
          else setGeneratedContents({}); // Clear if no cache on error
          // If quiz_progress was in the error response's full_cache, it will be set.
          // Reset currentQuizDisplayIndex if new quiz data is part of this error response.
        ...
        setGeneratedContents(data.full_cache || {});
        setActiveMode(mode);
        // If new quiz data is fetched (even from cache on success), reset display index
        if (mode === 'quiz' && data.full_cache?.quiz) {
          setCurrentQuizDisplayIndex(0);
        }
        if (isNewPrimaryFocus && mode === 'explain' && data.source === 'generated') { fetchProfileDataSilently(true); }
    ...
    const handleModeToggle = (newMode: ContentMode) => {
      ...
      setActiveMode(newMode);
      if (newMode === 'quiz' && oldActiveMode !== 'quiz') {
        // If generatedContents already has quiz data (and progress), use it, otherwise it will be fetched.
        // Resetting index here ensures we start from Q1 if data exists.
        setCurrentQuizDisplayIndex(0);
      }
      const contentIsMissing = newMode === 'quiz' ? !generatedContents.quiz : !generatedContents[newMode];
      const reviewContentMissing = isReviewingStreakWordRef.current && contentIsMissing;
      if (contentIsMissing || reviewContentMissing) {
        ...
        setCurrentQuizDisplayIndex(0); // Ensure first quiz question if data already exists
      }
    };
    ...
    const saveQuizAttemptToServer = async (questionIndex: number, selectedKey: string, isCorrect: boolean) => {
      ...
      if (!word || !user) {
        console.error("Cannot save quiz attempt: no word or user.");
        return;
      }
      try {
        ...
        // Optimistically update local quiz_progress
        setGeneratedContents(prevContents => {
          const newProgress = Array.isArray(prevContents.quiz_progress) ? [...prevContents.quiz_progress] : [];
          const attemptIndex = newProgress.findIndex(att => att.question_index === questionIndex);
          ...
          if (attemptIndex > -1) {
            newProgress[attemptIndex] = newAttemptEntry;
          } else {
            newProgress.push(newAttemptEntry);
            newProgress.sort((a, b) => a.question_index - b.question_index);
          }
          return { ...prevContents, quiz_progress: newProgress };
        });
    ...
    const handleQuizQuestionAnswered = (questionIndex: number, selectedKey: string, isCorrect: boolean) => {
      // This function is called by InteractiveQuiz when an answer is made.
      // It will then call the function to save the attempt to the server.
    ...
    const handleWordClickFromProfile = (word: string, cachedContentComplete?: Partial<GeneratedContent>) => {
      ...
      // When loading from profile, set generatedContents directly from cachedContentComplete
      // This ensures quiz_progress is also loaded if present in cache.
      setGeneratedContents(cachedContentComplete || {});
      setActiveMode(initialModeToLoad);
      // Call generateContent primarily to set lastSubmittedQuestionRef and potentially refresh if cache is stale/missing
      // but rely on cachedContentComplete for initial display.
      // Pass preserveCurrentStreak: true as the streak is handled by this function.
      generateContent(word, initialModeToLoad, { isNewPrimaryFocus: true, triggeredBy: 'profile_click', preserveCurrentStreak: true });

      if (initialModeToLoad === 'quiz' && cachedContentComplete?.quiz) {
        setCurrentQuizDisplayIndex(0); // Ensure first quiz of the set is shown
      }
      setShowProfileModal(false);
    };
    const handleReviewStreakWordClick = (wordFromStreak: string) => {
      ...
      const cachedContentsForWord = wordDataFromProfile?.generated_content_cache; // This includes quiz and quiz_progress
      ...
      if (cachedContentsForWord && Object.keys(cachedContentsForWord).length > 0) {
        ...
        // If not in profile cache (e.g. first time reviewing this word in streak), fetch its explain
    ...
    // Calculate quiz score based on saved progress
    ...
    const totalQuizzesInSet = generatedContents.quiz?.length || 0; // Total questions defined for the word
    const attemptedQuizzesInSet = currentQuizProgress.length; // Number of questions actually attempted

    let quizScoreDisplay = '';
    if (activeMode === 'quiz' && totalQuizzesInSet > 0) {
      if (attemptedQuizzesInSet === totalQuizzesInSet) {
        quizScoreDisplay = `Quiz Complete! Score: ${correctQuizAnswers}/${totalQuizzesInSet}`;
      } else if (attemptedQuizzesInSet > 0) {
        quizScoreDisplay = `Progress: ${correctQuizAnswers}/${attemptedQuizzesInSet} correct (out of ${totalQuizzesInSet} total questions)`;
      } else {
              ...
              if (activeMode === 'quiz') {
                if (currentQuizTextToShow) {
                  return (
                    <div className="mt-6 p-4 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-700 shadow">
                      <InteractiveQuiz
                        key={`${displayWord}-quiz-${currentQuizDisplayIndex}`} // Key change forces re-mount
                        quizText={currentQuizTextToShow}
                        onLoadNextQuestion={handleLoadNextQuizQuestion}
                        onQuestionAnswered={handleQuizQuestionAnswered}
                        canLoadNext={currentQuizArray ? currentQuizDisplayIndex < currentQuizArray.length - 1 : false}
                        questionNumber={currentQuizDisplayIndex + 1}
                        questionIndex={currentQuizDisplayIndex} // Pass 0-indexed
