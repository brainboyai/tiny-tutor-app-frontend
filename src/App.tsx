...
import {
  ...
  Settings, Menu, Plus, Flame, HelpCircle, Mic, BookText, FileText, Gamepad2, Lock
} from 'lucide-react';
import './App.css';
import './index.css';
...
// MODIFIED: Removed isLocked property, it's no longer needed
...
function App() {
  ...
  // NEW: Function to save guest state to sessionStorage
  const saveGuestStateToSession = () => {
      if (!authToken && liveStreak && liveStreak.words.length > 0) {
          const guestState = {
              liveStreak,
              liveStreakQuizQueue,
              currentFocusWord,
          ...
          sessionStorage.setItem('tiny-tutor-guest-state', JSON.stringify(guestState));
      }
  };
  ...
  const handleAuthAction = async (e: FormEvent) => {
      ...
      try {
          ...
          if (authMode === 'signup') {
              ...
              // NEW: Restore guest state after successful login
              const savedStateJSON = sessionStorage.getItem('tiny-tutor-guest-state');
              if (savedStateJSON) {
                  const savedState = JSON.parse(savedStateJSON);
                  setLiveStreak(savedState.liveStreak);
                  setLiveStreakQuizQueue(savedState.liveStreakQuizQueue);
                  setCurrentFocusWord(savedState.currentFocusWord);
                  setGeneratedContent(savedState.generatedContent);
                  setActiveTopic(savedState.activeTopic);
                  setIsInitialView(savedState.isInitialView);
                  sessionStorage.removeItem('tiny-tutor-guest-state'); // Clean up
              }
  ...
  const handleGenerateExplanation = useCallback(async (wordToFetch: string, isNewPrimaryWordSearch: boolean = false, isUserRefreshClick: boolean = false, isSubTopicClick: boolean = false) => {
    ...
    try {
        ...
        // MODIFIED: Quiz generation now fetches real quizzes for guests too
        if (explanationJustFetched && isNewWordBeingAddedToStreak) {
            try {
                const quizResp = await fetch(`${API_BASE_URL}/generate_explanation`, {
                    method: 'POST',
                    headers: headers, // Re-use headers (will be with or without auth)
  ...
  const handlePopupQuizAnswer = (optionKey: string) => { const currentItem = liveStreakQuizQueue[currentStreakQuizItemIndex]; if (!currentItem || currentItem.attempted) return; const isCorrect = currentItem.quizQuestion.correctOptionKey === optionKey; handleSaveQuizAttempt(currentItem.word, isCorrect); setLiveStreakQuizQueue(prev => prev.map((item, index) => index === currentStreakQuizItemIndex ? { ...item, attempted: true, selectedOptionKey: optionKey, isCorrect } : item )); };
   ...
   useEffect(() => {
    const storedLang = localStorage.getItem('tiny-tutor-language');
    if (storedLang) {
  ...
  const handleLanguageChange = (lang: string) => {
      setLanguage(lang);
      localStorage.setItem('tiny-tutor-language', lang);
  };

  const resetChat = useCallback(() => {
    const action = async () => {
      if (liveStreak && authToken && activeGameMode === 'explore_mode') {
        const newHistory = await saveStreakToServer(liveStreak, authToken);
        if (newHistory && userProfileData) {
          setUserProfileData(prev => ({...prev!, streakHistory: newHistory}));
        }
      }
      setInputValue('');
  ...
  }, [liveStreak, authToken, saveStreakToServer, userProfileData, activeGameMode]);
  ...
  // MODIFIED: Function to show auth modal now saves guest state first
  const showLoginModal = () => {
      saveGuestStateToSession();
  ...
  // In App.tsx, replace the existing renderQuizPopup function
...
const renderQuizPopup = () => {
    ...
    const { quizQuestion, attempted, selectedOptionKey } = currentItem;
      ...
      <div className="absolute top-4 right-4 w-full max-w-md bg-[--background-secondary] rounded-lg shadow-2xl p-6 z-20 border border-[--border-color] animate-fadeIn">
        <div className="flex justify-between items-center mb-4">
          <h3 className="font-semibold text-lg text-[--text-primary]">
            {!authToken ? "Quiz Preview" : "Quiz Time!"}
          </h3>
        ...
        <div className="space-y-2">
          {Object.entries(quizQuestion.options).map(([key, text]) => {
            ...
            // For guests, the button color is always the default.
            if (authToken && attempted) {
              if (key === quizQuestion.correctOptionKey) { buttonColor = "bg-green-800/80"; } 
              else if (key === selectedOptionKey) { buttonColor = "bg-red-800/80"; } 
              else { buttonColor = "bg-[--hover-bg-color] opacity-60"; }
            }
            return (
              <button
                key={key}
                onClick={() => handleOptionClick(key)}
                // CORRECTED: This now correctly evaluates to a boolean.
                // It disables the button only if the user is logged in AND has attempted the question.
                disabled={!!(authToken && attempted)}
                className={`w-full text-left p-3 rounded-md transition-colors ${buttonColor} ${!authToken ? 'cursor-pointer hover:bg-[--border-color]' : 'disabled:cursor-not-allowed'}`}
        ...
        {(authToken && attempted) && (
          <div className="flex justify-end mt-4">
            {isLastQuestion ? ( <button onClick={() => setIsQuizVisible(false)} className="bg-sky-600 hover:bg-sky-500 text-white font-semibold py-2 px-4 rounded-md"> Finish </button> ) 
            : ( <button onClick={() => setCurrentStreakQuizItemIndex(i => i + 1)} className="bg-[--accent-primary] text-black font-semibold py-2 px-4 rounded-md"> Next </button> )}
          </div>
        )}
      ...
      <div className="flex h-dvh w-full bg-[--background-default] text-[--text-primary] font-sans">
        ...
        <div className="flex flex-col flex-grow h-full max-h-screen">
          <header className="flex items-center p-2 pr-4 flex-shrink-0 border-b border-[--border-color]">
            ...
            <h1 className="text-lg font-medium ml-2 truncate">{activeView === 'profile' ? 'Profile' : (activeTopic || getDisplayWord() || "Tiny Tutor AI")}</h1>
            
            <div className="ml-4">
              <select 
                value={language} 
                onChange={(e) => handleLanguageChange(e.target.value)}
                className="bg-transparent border border-gray-600 rounded-md p-1 text-sm"
              >
            ...
            <div className="ml-auto">
              {!currentUser && (
                <button
                  onClick={showLoginModal} // MODIFIED: Use the new handler
                ...
                <div className="max-w-4xl mx-auto">
                  ...
                  >
                    <div className="flex items-center w-full">
                      ...
                      />
                      {inputValue.trim() && activeGameMode === null ? (
        ...
        {activeGameMode === 'explore_mode' && !isInitialView && unattemptedStreakQuizCount > 0 && (
          ...
          >
            {authToken ? <HelpCircle size={32} /> : <Lock size={28} />}
