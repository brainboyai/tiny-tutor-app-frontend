...
function MainApp() {
  ...
  const [startMode, setStartMode] = useState<'explore' | 'story' | 'game'>('explore');
...
// --- ADD THIS LINE HERE ---
  ...
  const handleLogout = useCallback(async () => { if (liveStreak && liveStreak.score >= 2 && authToken) { await saveStreakToServer(liveStreak, authToken); } localStorage.removeItem('authToken'); setAuthToken(null); setCurrentUser(null); setUserProfileData(null); setLiveStreak(null); setCurrentFocusWord(null); setGeneratedContent({}); setError(null); setAuthError(null); setAuthSuccessMessage(null); setShowAuthModal(false); navigate('/'); setLiveStreakQuizQueue([]); }, [liveStreak, authToken, saveStreakToServer, navigate]);
  const fetchUserProfile = useCallback(async (token: string | null) => { if (!token || profileFetchFailed) { setUserProfileData(null); setCurrentUser(null); return; } setIsFetchingProfile(true); try { const response = await fetch(`${API_BASE_URL}/profile`, { headers: { 'Authorization': `Bearer ${token}` }}); if (!response.ok) { if (response.status === 401) { handleLogout(); } else { setError(`Profile fetch failed: ${response.statusText}`);}  setProfileFetchFailed(true); return; } const data = await response.json(); const pE: ExploredWordEntry[]=(data.exploredWords||[]).map((w:any)=>(w&&typeof w.word==='string'?{...w}:null)).filter(Boolean); const pF:ExploredWordEntry[]=(data.favoriteWords||[]).map((w:any)=>(w&&typeof w.word==='string'?{...w}:null)).filter(Boolean); setUserProfileData({...data, exploredWords: pE, favoriteWords: pF, streakHistory: (data.streakHistory||[]).sort((a:any,b:any)=>new Date(b.completed_at).getTime()-new Date(a.completed_at).getTime())}); setCurrentUser({username:data.username, email:data.email, id:data.user_id||''}); setError(null); setProfileFetchFailed(false);} catch(e){console.error("Profile fetch error:", e); setError("Could not connect to the server."); setProfileFetchFailed(true);}finally{setIsFetchingProfile(false);}}, [profileFetchFailed, handleLogout]);
  ...
  const handleWordSelectionFromProfile = useCallback((word: string) => { const action = async () => { if (liveStreak && authToken) { const newHistory = await saveStreakToServer(liveStreak, authToken); if (newHistory && userProfileData) { setUserProfileData(prev => ({...prev!, streakHistory: newHistory})); } } navigate('/'); handleGenerateExplanation(word, true); }; if (liveStreak && liveStreak.score >= 2) { setWarningAction({ action }); setShowWarningModal(true); } else { action(); } }, [liveStreak, handleGenerateExplanation, saveStreakToServer, authToken, userProfileData, navigate]);
  ...
  const resetSession = useCallback(() => {
    const action = async () => {
      if (liveStreak && authToken) {
        ...
        if (newHistory && userProfileData) { setUserProfileData(prev => ({...prev!, streakHistory: newHistory})); }
      }
      setInputValue('');
    ...
    if (liveStreak && liveStreak.score >= 2) {
      setWarningAction({ action }); setShowWarningModal(true);
    } else { action(); }
  }, [liveStreak, authToken, saveStreakToServer, userProfileData, navigate]);
  ...
  const handleFormSubmit = (e: FormEvent) => {
    ...
    if (startMode === 'explore') {
      handleGenerateExplanation(inputValue, true);
    } else if (startMode === 'story') {
      navigate(`/story/${inputValue}`);
    } else if (startMode === 'game') {
  ...
  const renderExploreModeContent = () => {
    ...
    if (isLoading && !generatedContent[sanitizeWordForId(wordToUse!)]) { return <div className="text-center p-10 text-slate-400">Generating...</div>; }
    if (error) return <div className="text-center p-10 text-red-400">Error: {error}</div>;
    if (!wordToUse || !generatedContent[sanitizeWordForId(wordToUse)]) return null;
  ...
  const StoryModeWrapper = ({ onStoryEnd, authToken }: { onStoryEnd: (topic: string) => void, authToken: string | null }) => {
    const { topic } = useParams<{ topic: string }>();
    if (!topic) {
        useEffect(() => { navigate('/'); }, [navigate]);
        return <div>Topic not found! Redirecting...</div>;
    }
    return <StoryModeComponent topic={topic} authToken={authToken} onStoryEnd={() => onStoryEnd(topic)} />;
  };
      ...
      <div className="flex h-dvh w-full bg-[--background-default] text-[--text-primary] font-sans">
        <aside className={`bg-[--background-secondary] flex-shrink-0 transition-all duration-300 flex flex-col ${isSidebarOpen ? 'w-64 p-2' : 'w-0 p-0'} overflow-hidden`}>
          <div className="flex-grow space-y-1">
            {currentUser && (
              <button onClick={() => navigate('/profile')} className="flex items-center w-full p-2 rounded-md text-sm hover:bg-[--hover-bg-color]">
            ...
            <button onClick={resetSession} className="flex items-center w-full p-2 rounded-md text-sm hover:bg-[--hover-bg-color]"><Plus size={16} className="mr-3"/> New Session</button>
            {currentUser && (<><button onClick={() => navigate('/profile')} className="flex items-center w-full p-2 rounded-md text-sm hover:bg-[--hover-bg-color]"><User size={16} className="mr-3"/> Profile</button></>)}
          </div>
        ...
        <div className="flex flex-col flex-grow h-full max-h-screen">
          <header className="flex items-center p-2 pr-4 flex-shrink-0 border-b border-[--border-color]">
            ...
            <h1 className="text-lg font-medium ml-2 truncate">{
              location.pathname.startsWith('/profile') ? 'Profile' : 
              location.pathname.startsWith('/game') ? 'Game Mode' :
              location.pathname.startsWith('/story') ? 'Story Mode' :
              currentFocusWord || "Tiny Tutor AI"
            }</h1>
            <div className="ml-auto">{!currentUser && (<button onClick={() => { setShowAuthModal(true); setAuthMode('login');}} className="bg-[#a8c7fa] hover:bg-[#89b4fa] text-black font-semibold py-1.5 px-5 rounded-full transition-colors text-sm">Sign in</button>)}</div>
        </header>
            ...
            <Routes>
              <Route path="/" element={
                <div className="max-w-4xl mx-auto px-4 pt-8 pb-48">
                  {isInitialView ? (
                    <div className="text-center text-4xl font-medium text-slate-500 flex flex-col items-center justify-center h-full pt-16 animate-fadeIn"><span className="p-4 bg-sky-500/10 rounded-full mb-4"><Sparkles size={32} className="text-sky-400"/></span><span>Enter a Concept to Learn or Play!</span></div>
                  ) : (
                    <div className="space-y-6">
                        ...
                        <div className="flex items-center gap-2 flex-wrap p-3 bg-gradient-to-r from-slate-800/50 to-slate-900/20 rounded-xl border border-slate-700/50">
                          ...
                          {liveStreak.words.map((word, index) => (
                            <React.Fragment key={word + index}>
                              <button className={`py-1 px-3 rounded-full text-sm font-medium transition-all duration-300 shadow-md animate-fadeIn ${getDisplayWord() === word ? 'bg-sky-600 text-white' : 'bg-[--background-secondary] hover:bg-[--hover-bg-color]'}`} onClick={() => handleStreakWordClick(word)}>
                                {word}
                              </button>
                      ...
                      {renderExploreModeContent()}
                    </div>
                  )}
                </div>
              }/>
              <Route path="/story/:topic" element={<StoryModeWrapper onStoryEnd={(topic) => navigate(`/game/${topic}`)} authToken={authToken} />} />
              <Route path="/profile" element={<div className="max-w-4xl mx-auto px-4 pt-8 pb-48"><ProfilePageComponent currentUser={currentUser!} userProfileData={userProfileData} onWordSelect={handleWordSelectionFromProfile} onNavigateBack={() => navigate('/')} onToggleFavorite={handleToggleFavorite} /></div>} />
              <Route path="/game/:topic" element={<GameModeComponent authToken={authToken} />} />
            </Routes>
            <div ref={chatEndRef} />
            {renderQuizPopup()}
          </main>
          ...
          {location.pathname === '/' && (
            <div className={`absolute bottom-0 left-0 w-full p-4 bg-gradient-to-t from-[--background-default] to-transparent`}>
              <div className="max-w-4xl mx-auto">
                <form onSubmit={handleFormSubmit} className="bg-[--background-input] rounded-3xl p-4 flex flex-col shadow-lg border border-transparent focus-within:border-[--accent-primary] transition-colors">
                  <div className="flex items-center w-full">
                    <input type="text" value={inputValue} onChange={(e) => setInputValue(e.target.value)}
                      placeholder={
                        startMode === 'explore' ? "Enter a word to explore..." : 
                        startMode === 'story' ? "Enter a topic for a story..." : 
                        "Enter a topic for a game..."
                      }
                      className="w-full bg-transparent px-2 focus:outline-none text-lg"
                      // --- FIX: Logic to disable input ---
                      disabled={!isInitialView}
                    />
                    {inputValue.trim() ? (
                      <button type="submit" disabled={isLoading || !isInitialView} className="p-2 rounded-full bg-[--hover-bg-color] disabled:opacity-50 disabled:cursor-not-allowed">
                        <SendHorizontal size={22} />
                      </button>
                    ) : ( <div className="p-2 text-[--text-tertiary]"><Mic size={22} /></div> )}
                  </div>

                  <div className="flex items-center gap-4 mt-3">
                    {/* --- FIX: Logic to disable mode buttons --- */}
                    <button type="button" onClick={() => setStartMode('explore')} disabled={!isInitialView} className={`flex items-center gap-2 py-1 px-3 rounded-lg text-sm font-medium transition-colors ${startMode === 'explore' ? 'bg-[--accent-primary] text-black' : 'text-[--text-secondary] hover:bg-[--hover-bg-color]'} disabled:opacity-50`}><BookText size={16} /> Explore</button>
                    <button type="button" onClick={() => setStartMode('story')} disabled={!isInitialView} className={`flex items-center gap-2 py-1 px-3 rounded-lg text-sm font-medium transition-colors ${startMode === 'story' ? 'bg-[--accent-primary] text-black' : 'text-[--text-secondary] hover:bg-[--hover-bg-color]'} disabled:opacity-50`}><FileText size={16} /> Story</button>
                    <button type="button" onClick={() => setStartMode('game')} disabled={!isInitialView} className={`flex items-center gap-2 py-1 px-3 rounded-lg text-sm font-medium transition-colors ${startMode === 'game' ? 'bg-[--accent-primary] text-black' : 'text-[--text-secondary] hover:bg-[--hover-bg-color]'} disabled:opacity-50`}><Swords size={16} /> Game</button>
                  </div>
                </form>
              </div>
            </div>
        ...
        {renderWarningModal()}
        {unattemptedStreakQuizCount > 0 && location.pathname === '/' && <button onClick={() => setIsQuizVisible(true)} className="fixed bottom-28 right-10 z-30 flex items-center justify-center h-16 w-16 bg-sky-600 text-white rounded-full shadow-lg hover:bg-sky-500 transition-all transform hover:scale-110" title={`${unattemptedStreakQuizCount} questions available`}><HelpCircle size={32} /><span className="absolute -top-1 -right-1 bg-pink-500 text-white text-xs font-bold rounded-full h-6 w-6 flex items-center justify-center border-2 border-[--background-default]">{unattemptedStreakQuizCount}</span></button>}
      </div>
  );
}
